...
WORKS_PAGE = REPO_ROOT / "home" / "src" / "works" / "20230718-python-warts-and-all.md"
...
def update_works_page(chapter: dict, post_path: Path) -> None:
    """Replace the 'coming soon' line for this chapter with a link to the new post."""
    # Derive relative URL from works page to post (e.g. ../../posts/2026/02/slug/)
    parts = post_path.parts
    posts_idx = next(i for i, p in enumerate(parts) if p == "posts")
    rel_slug = "/".join(parts[posts_idx:]).removesuffix(".md")
    rel_url = f"../../{rel_slug}/"

    content = WORKS_PAGE.read_text()
    coming_soon = f"{chapter['num']}. {chapter['title']} *(coming soon)*"
    linked = f"{chapter['num']}. [{chapter['title']}]({rel_url})"

    if coming_soon not in content:
        print(f"Warning: could not find '{coming_soon}' in works page — skipping update", file=sys.stderr)
        return

    content = content.replace(coming_soon, linked)
    WORKS_PAGE.write_text(content)
    print(f"Updated works page: {WORKS_PAGE}")


def create_pr(chapter: dict, post_path: Path) -> None:
    """Create a git branch, commit the new post + works page update, push, and open a PR."""
    branch = f"claude/pwaa-chapter-{chapter['num']}-{chapter['slug']}"
    rel_post = post_path.relative_to(REPO_ROOT)
    rel_works = WORKS_PAGE.relative_to(REPO_ROOT)

    cmds = [
        ["git", "-C", str(REPO_ROOT), "checkout", "-b", branch],
        ["git", "-C", str(REPO_ROOT), "add", str(rel_post), str(rel_works)],
        [
            "git",
            "-C",
            str(REPO_ROOT),
...
def main() -> None:
    ...
    update_works_page(chapter, post_path)
